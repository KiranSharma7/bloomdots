...
- 2026-05-19 — enabled e_atomic_elements experiment. check-setup confirms runtime + style_schema + global_classes + variables + interactions all available; issues=[].
- 2026-05-20 - v4 variables created via Novamira variables API: 13 `e-gv-*` color tokens (`bg` through `bloom-amber`), including `bloom-orange` #E84E28, `bloom-blue` #29B9EA, and `bloom-amber` #FAB72B.
- 2026-05-20 - v4 global classes created via Novamira global-classes API with `g-bd-*` ids: bd-kicker, bd-eyebrow, bd-display, bd-cta-primary, bd-cta-ghost, bd-surface, bd-line-divider, bd-section-pad. Smoke page 204 rendered `.elementor .smoke{font-size:48px;color:#E84E28;}` in `local-204-frontend-desktop.css`; smoke page deleted after verification.
- 2026-05-20 - bloomdots v4 foundation plan complete. Gate re-checks passed: post 32 backup parses, sandbox palette has 13 vars/no legacy aliases, sandbox legacy hex/alias grep is clean, v3 kit and v4 tokens/classes are present, atomic setup reports issues=[], and the front page returns HTTP 200 with the expected BloomDots shell.
- 2026-05-20 - removed leftover token smoke page 202 (`token-smoke-draft`) during final cleanup; smoke-draft search now returns no pages.
